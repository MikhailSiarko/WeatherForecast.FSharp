...
module WeatherForecast =
    ...
    let private requestUpdateAsync apiKey (expired: ExpiredForecast) =
        async {
    ...
    let getAsync apiKey expiredAfter location =
        async {
            ...
            return
                ...
                | Some f ->
                    match Forecast.validate f DateTime.UtcNow expiredAfter with
                    | Valid (ValidForecast v) -> v
                    | Expired e ->
                        requestUpdateAsync apiKey e
                        |> Async.RunSynchronously
                | None ->
                    requestForecastAsync apiKey location
                    |> Async.RunSynchronously
        }
