...
let migrateUp (version: int64) (runner: IMigrationRunner) =
    match version with
    | x when x = Unchecked.defaultof<int64> ->
        if runner.HasMigrationsToApplyUp() then
            runner.MigrateUp()
    | v ->
        if runner.HasMigrationsToApplyUp(Nullable v) then
            runner.MigrateUp(v)
...
let migrateDown (version: int64) (runner: IMigrationRunner) =
    ...
    | v ->
        if runner.HasMigrationsToApplyDown(v) then
            runner.MigrateDown(v)

let run (action: IMigrationRunner -> unit) =
    let services = configureServices ()

    use scope =
        services.BuildServiceProvider(false).CreateScope()

    let runner =
        scope.ServiceProvider.GetService<IMigrationRunner>()
...
let fold (acc: StringBuilder) (error: Error) = acc.AppendLine(error.ToString())
...
let main argv =
    ...
    | :? (NotParsed<Options>) as notParsed ->
        failwith (
            (Seq.fold fold (StringBuilder()) notParsed.Errors)
                .ToString()
        )
    | _ -> failwith "Error"
